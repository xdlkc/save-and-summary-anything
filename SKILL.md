...
- 微信公众号、知乎、普通博客等文章页都可直接用本技能尝试抓取；只有当用户明确要求使用站点专用抓取器，或本技能抓取失败/质量明显不足时，才切换到 `wechat-article-fetch`、`twitter-article-fetch` 等专用技能。
...
1. 默认直接使用本技能脚本处理 URL；不要因为域名是微信公众号或 X/Twitter 就自动切换到专用技能，除非用户明确要求或本技能失败。
2. 运行 `save_article.py`。
3. 看脚本输出：
   - `ARTICLE_SAVED=<path>` 表示成功保存，读取该 Markdown。
   - `NOT_ARTICLE=...` 表示不是文字为主文章页，直接解释并停止。
4. 用保存的 Markdown 正文生成中文总结。默认采用信息密度较高的结构化格式：
   ...
   🧠 核心问题：<用一个问题句概括文章要解决什么>
   <1-2 段说明根本原因/背景/主要矛盾。保留文章中最有力量的原句或金句；引用用引号标出。>

   💡 <核心概念/方案> 是什么？
   <解释文章提出的核心方案、工具、方法或观点。>
   核心原则：<如果文中有原则/方法论，用一句话提炼。>

   🗂️ <关键结构/框架>
   <用短段或列表说明文章里的分层、分类、模型、架构。>

   ⚙️ <流程/方法>（自动完成/操作步骤/实践路径）
   1. <步骤一> — <保留具体动作和目的>
   2. <步骤二> — <保留具体动作和目的>
   3. <步骤三> — <保留具体动作和目的>

   🎮 使用建议 / 实践启发
   <总结用户应该怎么用、何时用、注意什么；保留数字、阈值、链接、案例。>

   开源地址/原文链接：<如文中有 GitHub/官网/项目地址则保留>
   ```

   生成要求：
   - 标题使用 `📰 文章总结`，后面给出书名号文章标题。
   - 尽量保留作者、日期；日期缺失时不要编造，可省略或写“日期未标明”。
   - 小标题可按文章内容改写，但优先使用 emoji + 明确主题，例如 `🧠 核心问题`、`💡 方案是什么`、`⚙️ 运行流程`、`🎮 使用建议`。
   - 保留具体事实、案例、数字、方法、结论、项目链接和关键原句；不要只写泛泛摘要。
   - 如果文章没有流程/框架，不要硬凑；改成“关键观点”“案例”“行动清单”等更贴合内容的小节。
   - 如果文章很长，先分块阅读再综合总结，最后输出一版去重后的结构化总结。
5. 最终回复包含：
   - Markdown 绝对路径
   - 图片数量（若脚本输出有）
   - 上述格式的中文总结
